...
pub fn draw(f: &mut Frame, app: &AppState) {
    let root = Layout::default()
        ...
        .constraints([
            Constraint::Length(1), // top title/hint bar
    ...
    draw_title_bar(f, root[0]);

    // Responsive: hide the sidebar on very narrow terminals so panes stay usable.
    let show_sidebar = root[1].width >= 64;
    let sidebar_w = if show_sidebar { (root[1].width / 5).clamp(20, 30) } else { 0 };
    ...
    let body = if show_sidebar {
        ...
        Layout::default().constraints([Constraint::Min(0)]).split(root[1])
    };
    ...
    let main_area = if show_sidebar { body[1] } else { body[0] };
...
// ─────────────────────────────────────────────────────────────────────────────
// Top title / hint bar — mimics a modern app titlebar with key hints
// ─────────────────────────────────────────────────────────────────────────────

fn draw_title_bar(f: &mut Frame, area: Rect) {
    let hints = [
        ("F1", "Help"), ("F2", "Rename"), ("F3", "View"), ("F5", "Copy"),
        ("F6", "Move"), ("Del", "Delete"), ("Esc", "Menu"),
    ];
    let mut spans = vec![
        Span::styled(" 🗂  ", Style::default().fg(C_ACCENT)),
        Span::styled("Rust File Manager", Style::default().fg(C_TEXT).add_modifier(Modifier::BOLD)),
        Span::raw("   "),
    ];
    for (i, (key, label)) in hints.iter().enumerate() {
        if i > 0 { spans.push(Span::raw("  ")); }
        spans.push(Span::styled(format!("{}", key), Style::default().fg(Color::Black).bg(C_ACCENT2)));
        spans.push(Span::styled(format!(" {}", label), Style::default().fg(C_MUTED)));
    }
    f.render_widget(
        Paragraph::new(Line::from(spans)).style(Style::default().bg(Color::Rgb(18, 19, 25))),
        area,
    );
}
...
fn draw_sidebar(f: &mut Frame, app: &AppState, area: Rect, geo: &mut LayoutGeometry) {
    ...
    push_line(f, geo, inner, &mut y, max_y,
        Line::from(Span::styled(" QUICK ACCESS", Style::default().fg(C_MUTED))), None);
    for (label, path) in app.quick_access.iter() {
        ...
        push_line(f, geo, inner, &mut y, max_y,
            Line::from(Span::styled(format!(" {}", label), style)), Some(path.clone()));
    }

    y += 1;
    push_line(f, geo, inner, &mut y, max_y,
        Line::from(Span::styled(" DRIVES", Style::default().fg(C_MUTED))), None);
    ...
    for d in app.drives.iter() {
        ...
        if d.total > 0 {
            ...
            push_line(f, geo, inner, &mut y, max_y,
                Line::from(vec![Span::styled(format!(" {}", bar), Style::default().fg(bar_color))]),
                None);
