...
*   **⚡ Responsive Navigation**: Coalesced pointer input, event-driven redraws, asynchronous directory scans/file operations/previews, live file-system watching, and four explicit preview policies: Normal, Full, Showcase, and Blitz.
...
*   **🖼️ Rich File Preview Engine**:
    ...
    *   **PDFs & Office**: Normal provides quiet extracted content without launching heavyweight renderers; Full uses LibreOffice/OpenOffice, Microsoft Office automation, Poppler, or MuPDF for exact pages; Showcase presents those exact renders in one consistent visual viewer; Blitz is zero-debounce and stays cache/metadata-first.
...
Preview modes are selected in the sidebar: **Normal** shows useful extracted content without starting an external renderer, **Full** forces exact Office/PDF page rendering, **Showcase** uses the same exact renderer through a consistent image-first document viewer, and **Blitz** applies the selection immediately with cached visuals or quiet lightweight metadata. The selected mode is authoritative; legacy Office/PDF settings cannot silently turn Normal into Full.
